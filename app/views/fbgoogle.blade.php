          ...
          @endif
            ...
            <form method="post" action="/postfbgoogle">
              <div class="input-wrapper custom-2">              
                <input class="form-control input-lg" type="text" placeholder="First Name" name="first_name" value="@if(isset($newUserData)){{$newUserData['first_name']}}@else{{Input::old('user_first_name')}}@endif" required>        
              </div>
               <div class="input-wrapper custom-2">
                <input class="form-control input-lg" type="text" placeholder="Last Name" name="last_name" value="@if(isset($newUserData)){{$newUserData['last_name']}}@else{{Input::old('user_first_name')}}@endif" required>        
              </div>
              <div class="input-wrapper">
                <i class="fa fa-user"></i>               
                <input class="form-control input-lg" type="email" placeholder="Email" name="email" value="@if(isset($newUserData)){{$newUserData['email']}}@endif" required>        
              </div>
              <div class="input-wrapper">
                <i class="fa fa-phone"></i>               
                <input class="form-control input-lg" placeholder="Contacts" name="mobno" value="{{Input::old('user_first_name')}}" required>        
              </div>
